...
3. 搭建TS运行环境
   ...
   2. 安装typescript
      ...
      2. 测试成功：tsc demo.ts
      3. 简化：npm install ts-node -g
      4. 测试：ts-node demo.ts

4. 基础类型   let demo: 类型 = 值；

   1. number，string，boolean，null ，undefined，symbol，void 
...
5. 对象类型  let demo: 类型 = 值；
   ...
   2. 数组
      ...
      ```js
      let arr: number[] = [1,2,3]  arr是一个数组 数组里都是number类型
      ```
...
6. 类型注解  ：我们来告诉ts ，这个变量是什么类型
7. 类型推断 ：ts会自动分析出来变量的类型
   1.  如果ts能够自动分析出来变量类型，就可以不加类型
   2.  如果ts不能够自动分析出来变量类型，那就使用【类型注解】
...
8. 函数类型注解
   ...
   // void ==> 不能有返回值 
   解构：fn({num1,str}:{num1:number,str:string})
   ```
